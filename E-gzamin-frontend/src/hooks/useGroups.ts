...
const useGroups = () => {
  const { data: groups, isFetching: isFetchingGroups, refetch: refetchGroups } = useQuery<
    Array<GroupType>,
    any,
    Error
  >('getGroups', getGroups, {
    ...
    const { data: ownedGroups, isFetching: isFetchingOwnedGroups, refetch: refetchOwnedGroups } = useQuery<
    Array<GroupType>,
    any,
    Error
  >('getOwnedGroups', getOwnedGroups, {
    ...
    const [createGroup] = useMutation<GroupType, {name: string, code: string}, Error>(
        async ({name, code}) => {
    ...
    return { groups: groups || [], ownedGroups: ownedGroups || [], createGroup, removeGroup, joinGroup, leaveGroup };
};
